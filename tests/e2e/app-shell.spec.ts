...
test('共通レイアウトをキーボードで利用でき、横方向に崩れない', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', { level: 1, name: 'Made for quieter days.' }),
  ).toBeVisible()
  await expect(page.getByRole('heading', { level: 2, name: 'New essentials' })).toBeVisible()
  const header = page.getByRole('banner')
  await expect(header.getByRole('link', { name: 'ALL ITEMS' })).toHaveAttribute('href', '/products')
  await expect(header.getByRole('link', { name: 'ログイン' })).toHaveAttribute('href', '/login')
  await expect(header.getByRole('link', { name: /SEASONAL EDIT/u })).toHaveCount(0)
  await expect(header.getByRole('link', { name: /POINT OF VIEW/u })).toHaveCount(0)
  ...
  if (test.info().project.name === 'chromium-375') {
    ...
    await expect(page.getByRole('link', { name: 'カート' })).toHaveAttribute('href', '/cart')
    ...
    await expect(page.getByRole('link', { name: '注文履歴' })).toHaveAttribute('href', '/orders')
    await expectHeaderControlsNotToOverlap(page)

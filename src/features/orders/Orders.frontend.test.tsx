...
describe('注文履歴', () => {
  it('ORDER-010: 空状態から商品一覧へ移動できる', () => {
    ...
    expect(
      screen.getByRole('link', { name: '商品一覧を見る' }),
  ...
  it('履歴からsnapshot注文の詳細へ移動できる', () => {
    ...
    expect(
      screen.getByRole('link', { name: '注文詳細を見る' }),
    ).toHaveAttribute('href', `/orders/${orderFixture.id}`)
  })
})
...
describe('注文詳細と完了', () => {
  ...
  it('注文完了を通知し注文履歴への導線を表示する', () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'ご注文を受け付けました' }),
    ).toBeVisible()
    expect(screen.getByRole('status')).toHaveTextContent(
      '注文内容を保存しました',
    )
    expect(
      screen.getByRole('link', { name: '注文履歴を見る' }),
    ).toHaveAttribute('href', '/orders')
  })
  ...
  ])('$name画面のerror境界からresetを実行できる', async ({
    Component,
    expected,
  }) => {
    const reset = vi.fn()

...
- UIの操作・状態・エラー、設計文書、テストの利用者向け説明は日本語。トップページでVite試作から引き継ぐブランド名、シーズン名、編集見出しと `ALL ITEMS` は英語表記を維持する。認証・アカウント操作は `ログイン`、`マイページ`、`オーダー`、`商品管理`、`ログアウト` と日本語で表示し、カートは読み上げ名 `カート` のアイコンで表示する

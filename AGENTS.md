...
- UIの操作・状態・エラー、設計文書、テストの利用者向け説明は日本語。トップページでVite試作から引き継ぐブランド名、シーズン名、編集見出しと `ALL ITEMS` は英語表記を維持する。認証・アカウント操作は `ログイン`、`マイページ`、`注文履歴`、`商品管理`、`ログアウト` と日本語で表示し、カートは読み上げ名 `カート` のアイコンで表示する
...
複数ファイルの変更、機能追加、設計変更、大規模なリファクタリングでは、
実装前に実行計画を作成する。

計画を作成する前に、計画が必要と判断した理由と保存予定の
`docs/plans/<task-name>.md` を示し、作成してよいかユーザーに確認する。

許可を得た後、`docs/PLANNING.md` に従って実行計画を作成する。

計画を作成した時点では実装を開始せず、ユーザーの確認を待つ。
承認後は、その計画を参照して実装する。

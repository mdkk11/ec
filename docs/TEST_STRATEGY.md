...
- 商品詳細だけに正確な在庫数が表示され、公開一覧は在庫状態だけを表示すること
- カートが契約の実商品画像を表示し、現在庫以内のnative selectを作ること
- 数量選択時の即時送信、在庫超過の保存数量を自動補正しない表示、在庫0・非公開時の操作制限
- 数量更新の400では明示的な最新カート取得、500・通信失敗では同じ希望数量の再試行を提供し、それまでは選択値と確定済み合計を保持すること
- 送信中の同一操作を重複送信せず、完了後は再操作できること
- 競合後に最新データを再取得する操作
- 直列queueで実行中操作と最新の未実行操作が並んでも、古い結果で新しい選択を上書きしないこと
- role別headerのカートアイコン、マイページ開閉、導線、logout中・失敗状態
- 管理商品一覧が管理DTOの実画像と既存metadata・編集導線・空状態を表示すること
...
- 商品詳細の現在庫、カートの実商品画像、在庫内の数量選択が実API契約から一体として表示・更新される代表例
- 在庫条件が変わった注文の失敗と回復
- 管理者の商品作成・編集・非公開化
- 管理者の在庫更新と注文状態更新
...
- 商品詳細: 現在庫数を含む通常、在庫0、長い商品名・説明
- カート: 実商品画像と数量selectを含む通常、縦並び導線の空、更新中、在庫超過
- 共通header: 匿名、購入者メニューopen、管理者メニューopen
- 管理商品一覧: 実画像を含む通常、空
...
| カート数量selectの400/500回復とrequest race | フロントエンド結合 | E2Eでは在庫内の即時更新を代表1例だけ |
| 商品詳細・カートの画像とレイアウト | VRT | Front結合ではalt、選択肢、導線を確認 |

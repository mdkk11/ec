...
- PR差分から責任のあるjobだけを実行し、E2Eはdependency graphと明示mapから影響specを選ぶ。分類不能・共通基盤・解析失敗と `main` pushは安全側の全実行へ倒す。
...
- テストコードに理由のない `.skip`、`.only`、retry、外部ネットワーク依存がなく、CIのjob skipは差分選択結果と理由を追跡できる。

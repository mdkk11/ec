...
| `AUTH-013` | 匿名・購入者・管理者のheader | マイページをkeyboardで開きrole別導線を確認 | 匿名はログイン、購入者はカートとオーダー、管理者は商品管理とオーダーを利用でき、logout中・失敗も通知する | Front結合 | E2E / VRT |
...
| `PRODUCT-008` | 在庫0の公開商品 | 商品詳細を表示 | `在庫 0点` と表示し、カート追加を利用不可にする | Front結合 | VRT |
...
| `CART-003` | 表示時在庫4、保存数量2、更新前に在庫3へ減少 | selectで数量4を選択 | 400 `QUANTITY_EXCEEDS_STOCK` 後も選択4と確定済み合計を維持し、明示的な再取得後に選択肢を1〜3へ更新 | Front結合 | Backend結合 |
...
| `CART-006` | カートが空 | カート画面を表示 | 空状態と商品一覧・注文履歴への導線を中央の縦並びと共通gapで表示 | Front結合 | VRT |
| `CART-007` | 在庫3、数量1、更新APIが保留中 | selectで数量2を選択し、同じ更新を重ねて要求 | 選択時にPATCHを即時送信し、同一内容の保留中requestは1件にまとめて更新中を通知 | Front結合 | VRT |
| `CART-008` | 数量1、希望数量2への更新APIが500または通信失敗 | 数量2を選択して失敗後に再試行 | 選択2と確定済み合計を維持し、エラー時だけ同じ希望数量の再試行を表示して成功後に同期 | Front結合 | - |
| `CART-009` | 数量2への更新APIが保留中 | 更新中に数量3を選択し、先行requestが成功または失敗 | 最新希望3を次に送信し、古い応答で選択3・新しいerror・確定結果を上書きしない | Front結合 | - |
...
| `CART-014` | ログイン済み、公開・在庫ありの商品詳細 | カート追加を送信中に連打し、成功後に再度追加 | 送信中は商品追加APIを1回だけ送り、成功後は操作を再び利用可能にする | Front結合 | E2E |

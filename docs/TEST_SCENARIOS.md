...
| `CART-007` | 更新APIが保留中 | 同じ数量へ再度更新 | 同一内容の重複送信を無効化し更新中を通知。異なる新しい数量は最新操作として受け付ける | Front結合 | VRT |
| `CART-008` | 更新APIが500 | 数量変更 | 入力を勝手に確定せず再試行可能 | Front結合 | - |
| `CART-009` | 数量2の更新APIが保留中 | 更新中に異なる新しい数量3へ変更 | 先行更新後に最新希望数量3を送信し、画面とサーバーを数量3へ一致させる | Front結合 | - |
